...
call git apply "%COMMAND_DIR%/patches/h264_multithread_windows.patch" -v --ignore-space-change --ignore-whitespace --whitespace=nowarn
call git apply "%COMMAND_DIR%/patches/h264_decoder_multithread_windows.patch" -v --ignore-space-change --ignore-whitespace --whitespace=nowarn
call git apply "%COMMAND_DIR%/patches/h264_encoder_params.patch" -v --ignore-space-change --ignore-whitespace --whitespace=nowarn

cd ..

if not exist "%ARTIFACTS_DIR%\lib" mkdir "%ARTIFACTS_DIR%\lib"
...
if exist "%OUTPUT_DIR%\obj\webrtc.lib" (
  copy "%OUTPUT_DIR%\obj\webrtc.lib" "%ARTIFACTS_DIR%\lib"
) else if exist "%OUTPUT_DIR%\obj\libwebrtc.lib" (
  copy "%OUTPUT_DIR%\obj\libwebrtc.lib" "%ARTIFACTS_DIR%\lib\webrtc.lib"
) else if exist "%OUTPUT_DIR%\obj\webrtc\webrtc.lib" (
  copy "%OUTPUT_DIR%\obj\webrtc\webrtc.lib" "%ARTIFACTS_DIR%\lib"
) else (
  echo ERROR: webrtc.lib not found in obj\. Searching...
  dir /s /b "%OUTPUT_DIR%\*.lib" 2>nul
  exit /b 1
)
