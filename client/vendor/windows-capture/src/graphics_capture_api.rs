...
pub struct GraphicsCaptureApi {
    ...
    /// Pixel format used for the frame pool (stored for Recreate-after-StartCapture workaround).
    _pool_pixel_format: DirectXPixelFormat,
    /// MinUpdateInterval in µs (stored to re-apply after Recreate; some systems need it post-StartCapture).
    _min_update_interval_us: Option<u64>,
}
...
impl GraphicsCaptureApi {
    ...
    >(
        ...
        let min_update_interval_us = if minimum_update_interval_settings != MinimumUpdateIntervalSettings::Default {
            if Self::is_minimum_update_interval_supported()? {
                match minimum_update_interval_settings {
                    MinimumUpdateIntervalSettings::Default => None,
        ...
        Ok(Self {
            ...
            _pool_pixel_format: pixel_format,
            _min_update_interval_us: min_update_interval_us,
        })
    }
    ...
    pub fn start_capture(&mut self) -> Result<(), Error> {
        ...
        // WGC workaround: if FramePool was created before StartCapture, the capture can get stuck
        // at 30 FPS on some systems. Recreating the pool after StartCapture avoids this.
        if let Some(ref pool) = self.frame_pool {
            let size = self.item.Size()?;
            pool.Recreate(&self._direct3d_device, self._pool_pixel_format, 3, size)?;
        }

        // Re-apply MinUpdateInterval after Recreate; on some systems the pool Recreate can
        // reset throttling to default (0 → ~50 fps or half-rate 30 fps).
        if let Some(us) = self._min_update_interval_us {
            if Self::is_minimum_update_interval_supported()? {
                let session = self.session.as_ref().unwrap();
                session.SetMinUpdateInterval(Duration::from_micros(us).into())?;
            }
        }
