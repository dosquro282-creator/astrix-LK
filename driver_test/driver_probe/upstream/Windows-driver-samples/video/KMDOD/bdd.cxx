                                                                                        ...
                                                                                        m_AdapterPowerState(PowerDeviceD0)
{
    ...
    *((UINT*)&m_Flags) = 0;
    ...
    BDD_PROBE1("BASIC_DISPLAY_DRIVER constructed pdo=0x%I64x",
               (ULONGLONG)(ULONG_PTR)m_pPhysicalDevice);
}
                                           ...
                                           _Out_ ULONG*             pNumberOfChildren)
{
    ...
    BDD_PROBE3("StartDevice core pdo=0x%I64x dxgkDeviceHandle=0x%I64x interfaceVersion=0x%x",
               (ULONGLONG)(ULONG_PTR)m_pPhysicalDevice,
               (pDxgkInterface != NULL) ? (ULONGLONG)(ULONG_PTR)pDxgkInterface->DeviceHandle : 0,
               (pDxgkInterface != NULL) ? pDxgkInterface->Version : 0);
    ...
    BDD_PROBE2("StartDevice got device information deviceHandle=0x%I64x pdo=0x%I64x",
               (ULONGLONG)(ULONG_PTR)m_DxgkInterface.DeviceHandle,
               (ULONGLONG)(ULONG_PTR)m_pPhysicalDevice);
...
//    Status = CheckHardware();
    ...
    BDD_PROBE5("StartDevice acquired POST display target=%u %ux%u pitch=%u format=%u",
               m_CurrentModes[0].DispInfo.TargetId,
               m_CurrentModes[0].DispInfo.Width,
               m_CurrentModes[0].DispInfo.Height,
               m_CurrentModes[0].DispInfo.Pitch,
               m_CurrentModes[0].DispInfo.ColorFormat);
    BDD_PROBE2("StartDevice POST framebuffer physical=0x%I64x cpuMappedPtr=0x%I64x",
               (ULONGLONG)m_CurrentModes[0].DispInfo.PhysicAddress.QuadPart,
               (ULONGLONG)(ULONG_PTR)m_CurrentModes[0].FrameBuffer.Ptr);
...
NTSTATUS BASIC_DISPLAY_DRIVER::StopDevice(VOID)
{
    ...
    BDD_PROBE0("StopDevice core cleanup requested");
    CleanUp();
...
NTSTATUS BASIC_DISPLAY_DRIVER::QueryAdapterInfo(_In_ CONST DXGKARG_QUERYADAPTERINFO* pQueryAdapterInfo)
{
    ...
    BDD_PROBE4("QueryAdapterInfo core type=%u pInput=0x%I64x pOutput=0x%I64x outputBytes=%u",
               pQueryAdapterInfo->Type,
               (ULONGLONG)(ULONG_PTR)pQueryAdapterInfo->pInputData,
               (ULONGLONG)(ULONG_PTR)pQueryAdapterInfo->pOutputData,
    ...
    switch (pQueryAdapterInfo->Type)
    {
        case DXGKQAITYPE_DRIVERCAPS:
        {
            ...
            BDD_PROBE2("QueryAdapterInfo DRIVER_CAPS wddmVersion=0x%x highestAddress=0x%I64x",
                       pDriverCaps->WDDMVersion,
                       (ULONGLONG)pDriverCaps->HighestAcceptableAddress.QuadPart);
            return STATUS_SUCCESS;
        }
        ...
        case DXGKQAITYPE_DISPLAY_DRIVERCAPS_EXTENSION:
        {
            ...
            BDD_PROBE1("QueryAdapterInfo DISPLAY_DRIVERCAPS_EXTENSION virtualModeSupport=%u",
                       pDriverDisplayCaps->VirtualModeSupport);
            return STATUS_SUCCESS;
        }
...
NTSTATUS BASIC_DISPLAY_DRIVER::PresentDisplayOnly(_In_ CONST DXGKARG_PRESENT_DISPLAYONLY* pPresentDisplayOnly)
{
    ...
    BOOLEAN FrameBufferBelongsToThisDriver =
        (pPresentDisplayOnly->VidPnSourceId < MAX_VIEWS) &&
        m_CurrentModes[pPresentDisplayOnly->VidPnSourceId].Flags.FrameBufferIsActive;

    BDD_PROBE5("PresentDisplayOnly core sourceId=%u source=0x%I64x bytesPerPixel=%u pitch=%u dirtyRects=%u",
               pPresentDisplayOnly->VidPnSourceId,
               (ULONGLONG)(ULONG_PTR)pPresentDisplayOnly->pSource,
               pPresentDisplayOnly->BytesPerPixel,
               pPresentDisplayOnly->Pitch,
               pPresentDisplayOnly->NumDirtyRects);
    BDD_PROBE5("PresentDisplayOnly core moves=%u rotate=%u dstCpuPtr=0x%I64x dstPhysical=0x%I64x belongsToThisDriver=%u",
               pPresentDisplayOnly->NumMoves,
               pPresentDisplayOnly->Flags.Rotate,
               FrameBufferBelongsToThisDriver ? (ULONGLONG)(ULONG_PTR)m_CurrentModes[pPresentDisplayOnly->VidPnSourceId].FrameBuffer.Ptr : 0,
               (pPresentDisplayOnly->VidPnSourceId < MAX_VIEWS) ? (ULONGLONG)m_CurrentModes[pPresentDisplayOnly->VidPnSourceId].DispInfo.PhysicAddress.QuadPart : 0,
               FrameBufferBelongsToThisDriver);
    BDD_PROBE0("PresentDisplayOnly source pointer is CPU-readable by display-only DDI contract; it is not another vendor adapter VRAM pointer");
...
{
    BDD_PROBE1("DpcRoutine core notifying dxgkrnl deviceHandle=0x%I64x",
               (ULONGLONG)(ULONG_PTR)m_DxgkInterface.DeviceHandle);
...
BOOLEAN BASIC_DISPLAY_DRIVER::InterruptRoutine(_In_  ULONG MessageNumber)
{
    ...
    BDD_PROBE1("InterruptRoutine core message=%u ignored; KMDOD has no hardware interrupts", MessageNumber);
    return FALSE;
}
