...
NTSTATUS BASIC_DISPLAY_DRIVER::IsSupportedVidPn(_Inout_ DXGKARG_ISSUPPORTEDVIDPN* pIsSupportedVidPn)
{
    ...
    BDD_PROBE1("IsSupportedVidPn core desiredVidPn=0x%I64x",
               (ULONGLONG)(ULONG_PTR)pIsSupportedVidPn->hDesiredVidPn);
    ...
    pIsSupportedVidPn->IsVidPnSupported = TRUE;
    BDD_PROBE1("IsSupportedVidPn core supported=%u", pIsSupportedVidPn->IsVidPnSupported);
    return STATUS_SUCCESS;
}
...
NTSTATUS BASIC_DISPLAY_DRIVER::RecommendFunctionalVidPn(_In_ CONST DXGKARG_RECOMMENDFUNCTIONALVIDPN* CONST pRecommendFunctionalVidPn)
{
    ...
    BDD_PROBE1("RecommendFunctionalVidPn core request=0x%I64x returning no recommendation",
...
NTSTATUS BASIC_DISPLAY_DRIVER::CommitVidPn(_In_ CONST DXGKARG_COMMITVIDPN* CONST pCommitVidPn)
{
    ...
    BDD_PROBE4("CommitVidPn core hFunctionalVidPn=0x%I64x affectedSource=%u pathPoweredOff=%u monitorChecks=%u",
               (ULONGLONG)(ULONG_PTR)pCommitVidPn->hFunctionalVidPn,
               pCommitVidPn->AffectedVidPnSourceId,
               pCommitVidPn->Flags.PathPoweredOff,
               pCommitVidPn->MonitorConnectivityChecks);
    BDD_PROBE1("CommitVidPn primary allocation ignored by display-only driver hPrimaryAllocation=0x%I64x",
               (ULONGLONG)(ULONG_PTR)pCommitVidPn->hPrimaryAllocation);
    ...
    BDD_PROBE2("CommitVidPn topology paths=%Iu pathsFromAffectedSource=%Iu",
               NumPaths,
               NumPathsFromSource);
    ...
    for (SIZE_T PathIndex = 0; PathIndex < NumPathsFromSource; ++PathIndex)
    {
        ...
        BDD_PROBE5("CommitVidPn path source=%u target=%u rotation=%u scaling=%u importance=%u",
                   pVidPnPresentPath->VidPnSourceId,
                   pVidPnPresentPath->VidPnTargetId,
                   pVidPnPresentPath->ContentTransformation.Rotation,
                   pVidPnPresentPath->ContentTransformation.Scaling,
                   pVidPnPresentPath->ImportanceOrdinal);
...
CommitVidPnExit:
    ...
    BDD_PROBE2("CommitVidPn core exit status=0x%08x affectedSource=%u",
               Status,
               pCommitVidPn->AffectedVidPnSourceId);
...
NTSTATUS BASIC_DISPLAY_DRIVER::UpdateActiveVidPnPresentPath(_In_ CONST DXGKARG_UPDATEACTIVEVIDPNPRESENTPATH* CONST pUpdateActiveVidPnPresentPath)
{
    ...
    BDD_PROBE4("UpdateActiveVidPnPresentPath source=%u target=%u rotation=%u scaling=%u",
               pUpdateActiveVidPnPresentPath->VidPnPresentPathInfo.VidPnSourceId,
               pUpdateActiveVidPnPresentPath->VidPnPresentPathInfo.VidPnTargetId,
               pUpdateActiveVidPnPresentPath->VidPnPresentPathInfo.ContentTransformation.Rotation,
               pUpdateActiveVidPnPresentPath->VidPnPresentPathInfo.ContentTransformation.Scaling);
                                                    ...
                                                    CONST D3DKMDT_VIDPN_PRESENT_PATH* pPath)
{
    ...
    BDD_PROBE5("SetSourceModeAndPath source=%u target=%u src=%ux%u format=%u",
               pPath->VidPnSourceId,
               pPath->VidPnTargetId,
               pCurrentBddMode->SrcModeWidth,
               pCurrentBddMode->SrcModeHeight,
               pSourceMode->Format.Graphics.PixelFormat);
    BDD_PROBE5("SetSourceModeAndPath display=%ux%u pitch=%u colorFormat=%u physical=0x%I64x",
               pCurrentBddMode->DispInfo.Width,
               pCurrentBddMode->DispInfo.Height,
               pCurrentBddMode->DispInfo.Pitch,
               pCurrentBddMode->DispInfo.ColorFormat,
               (ULONGLONG)pCurrentBddMode->DispInfo.PhysicAddress.QuadPart);
    ...
    if (!pCurrentBddMode->Flags.DoNotMapOrUnmap)
    {
        ...
        BDD_PROBE3("SetSourceModeAndPath map framebuffer status=0x%08x cpuPtr=0x%I64x bytes=%u",
                   Status,
                   (ULONGLONG)(ULONG_PTR)pCurrentBddMode->FrameBuffer.Ptr,
                   pCurrentBddMode->DispInfo.Pitch * pCurrentBddMode->DispInfo.Height);
    }
    ...
    BDD_PROBE2("SetSourceModeAndPath result status=0x%08x frameBufferActive=%u",
               Status,
               pCurrentBddMode->Flags.FrameBufferIsActive);
